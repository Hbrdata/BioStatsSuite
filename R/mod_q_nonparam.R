#' q_nonparam UI Function
#'
#' @description A shiny Module for non-parametric tests.
#'
  ...
  tagList(
    tags$div(
      ...
      selectInput(ns("test_in"), "组内检验",
                  choices = c("不进行" = "", "配对t检验" = "1", "Wilcoxon符号秩" = "2"),
                  selected = "",
                  width = "100%"),

      # 组间检验
      selectInput(ns("test_between"), "组间检验",
                  choices = c("不进行" = "", "参数检验" = "1", "非参数检验" = "2"),
                  selected = "",
                  width = "100%"),
...
#' q_nonparam Server Functions
#'
#' @noRd
mod_q_nonparam_server <- function(id, data_upload_module) {
  moduleServer(id, function(input, output, session) {
    ...
    clear_parameters <- function() {
      ...
      updateTextInput(session, "table_title", value = "非参数检验分析结果")
      updateTextInput(session, "ftnote", value = "")

      # 重置选择输入
      updateSelectInput(session, "test_in", selected = "")
      updateSelectInput(session, "test_between", selected = "")
    ...
    observe({
      ...
      if (!is.null(data_upload_module()$current_data) &&
        ...
        if (needs_reinit && nrow(current_data) > 0) {
          ...
          message("非参数检验初始化完成，分析类型: ", current_analysis_type)
        }
      } else {
        # 数据为空时，只更新状态，不清空参数输入
        if (rv$init_done) {
    ...
    return(reactive({
      ...
      # 转换检验选项为整数，空字符串表示不进行(NULL)
      test_in_val <- input$test_in
      test_between_val <- input$test_between
      ...
      list(
        ...
        test_in = if (test_in_val == "") NULL else as.integer(test_in_val),
        test_between = if (test_between_val == "") NULL else as.integer(test_between_val),
...
# mod_q_nonparam_ui("q_nonparam_1")

## To be copied in the server
# mod_q_nonparam_server("q_nonparam_1")
