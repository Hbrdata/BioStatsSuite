  ...
  tagList(
    tags$div(
      ...
      textInput(ns("var_mapping"), "变量映射", value = "",
                placeholder = "格式: 值1=标签1/值2=标签2/...（第一个值为阳性事件）",
                width = "100%"),

      # 分层变量选择
      tags$div(
        style = "margin-bottom: 15px;",
        selectizeInput(ns("cmhvar"), "分层变量(可选)",
                       choices = NULL,
                       multiple = FALSE,
                       options = list(placeholder = '选择分层变量（可选）',
                                      maxItems = 1))
      ),

      # 计算方法
      selectInput(ns("method"), "计算方法",
                  choices = c("Wald正态近似(MH)" = "MH",
                              "Newcombe-Wilson" = "Newcombe"),
                  selected = "MH",
                  width = "100%"),
...
#' riskdiff Server Functions
#'
#' @noRd
mod_riskdiff_server <- function(id, data_upload_module){
  moduleServer(id, function(input, output, session){
    ...
    clear_parameters <- function() {
      ...
      updateTextInput(session, "table_title", value = "率差分析表")
      updateTextInput(session, "ftnote", value = "")

      # 重置选择输入
      updateSelectInput(session, "method", selected = "MH")
    ...
    observe({
      ...
      if (!is.null(data_upload_module()$current_data) &&
        ...
        if (needs_reinit && nrow(current_data) > 0) {
          ...
          updateSelectizeInput(session, "cmhvar", choices = c("", vars), selected = "")

          # 设置默认值（大小写不敏感检测）
          if (current_analysis_type == "riskdiff") {
            message("设置率差分析默认变量...")
...
# mod_riskdiff_ui("riskdiff_1")

## To be copied in the server
# mod_riskdiff_server("riskdiff_1")
