...
mod_c_cmh_ui <- function(id) {
  ...
  tagList(
    tags$div(
      ...
      tags$div(
        style = "margin-bottom: 15px;",
        selectizeInput(ns("cmhvar"), "CMH分层变量",
                       choices = NULL,
                       multiple = FALSE,
                       options = list(placeholder = '选择CMH分层变量（如中心）'))
      ),

      # CMH方法选择
      selectInput(ns("method"), "CMH方法",
                  choices = c("非零相关" = "1", "行均值得分差值" = "2", "一般关联" = "3"),
                  selected = "1",
                  width = "100%"),

      # 显示选项
      checkboxInput(ns("outp"), "输出统计量和P值", value = TRUE),
...
#' c_cmh Server Functions
#'
#' @noRd
mod_c_cmh_server <- function(id, data_upload_module){
  moduleServer(id, function(input, output, session){
    ...
    clear_parameters <- function() {
      ...
      updateSelectizeInput(session, "cmhvar", selected = "")

      # 清空文本输入
      updateTextInput(session, "var_label", value = "")
      updateTextInput(session, "var_mapping", value = "")
      updateTextInput(session, "table_title", value = "CMH卡方检验分析结果")
      updateTextInput(session, "ftnote", value = "")

      # 重置下拉选择
      updateSelectInput(session, "method", selected = "1")
    ...
    observe({
      ...
      if (!is.null(data_upload_module()$current_data) &&
        ...
        if (needs_reinit && nrow(current_data) > 0) {
          ...
          updateSelectizeInput(session, "cmhvar", choices = vars, selected = "")

          # 只有当是CMH卡方检验时才设置默认值
          if (current_analysis_type == "c_cmh") {
            # 大小写不敏感的变量查找
            ...
            if (!is.null(detected_group)) {
              message("设置CMH卡方检验默认变量... 分组变量: ", detected_group)
            ...
            # 检测分析变量 (sfgs_tb)
            detected_var <- find_var(vars, c("sfgs_tb", "SFGS_TB"))
            if (!is.null(detected_var)) {
              updateSelectizeInput(session, "var_name", selected = detected_var)
            }

            # 检测CMH分层变量 (cn)
            detected_cmhvar <- find_var(vars, c("cn", "CN"))
            if (!is.null(detected_cmhvar)) {
              updateSelectizeInput(session, "cmhvar", selected = detected_cmhvar)
            }

            # 设置方法默认值为3（一般关联）
            updateSelectInput(session, "method", selected = "3")
          }
    ...
    observe({
      ...
      if (input$var_mapping == "") {
        ...
        if (var_name %in% names(current_data)) {
          ...
          unique_values <- unique_values[nchar(trimws(unique_values)) > 0]
          if (length(unique_values) <= 5) {
            # 预定义的变量映射标签
            label_map <- list(
              "sfgs_tb" = list("是" = "改善", "否" = "未改善"),
              "SFGS_TB" = list("是" = "改善", "否" = "未改善")
            )
            ...
            for (val in sort(unique_values)) {
              # 使用预定义标签或默认使用值本身
              if (var_name %in% names(label_map) && val %in% names(label_map[[var_name]])) {
                display_label <- label_map[[var_name]][[val]]
              } else {
                display_label <- val
              }
              mapping_parts <- c(mapping_parts, paste0("'", val, "'=", display_label))
            }
    ...
    return(reactive({
      ...
      varlist_text <- if (!is.null(input$var_label) && input$var_label != "") {
        if (!is.null(input$var_mapping) && input$var_mapping != "") {
      ...
      list(
        ...
        cmhvar = if (!is.null(input$cmhvar) && input$cmhvar != "") input$cmhvar else "",
        method = as.integer(input$method),
...
# mod_c_cmh_ui("c_cmh_1")

## To be copied in the server
# mod_c_cmh_server("c_cmh_1")
